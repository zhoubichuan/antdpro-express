...
// 添加规则
router.post('/rule1', async (req, res) => {
  let {
    code = Math.random(),
    work = 'xxx',
    area = 100,
  ...
  let target = await RuleModel1.find({ code })
  if (target.length) {
    res.send({ status: 'error', message: '规则重复' });
    return
  }
  let result = await RuleModel1.create({ code, work, area, mileage, time,workStatus, assessStatus, option});
  return res.json(result);
});

//查询
// router.get('/rule', checkLogin, async (req, res) => {
  router.get('/rule1', async (req, res) => {
  ...
  let total = await RuleModel1.countDocuments(query);
  let users = await RuleModel1.find(query)
